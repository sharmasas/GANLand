...
/** PAGE SETUP **/
let session1 = [];
// session.push(JSON.parse(sessionStorage.getItem('session')));
// sessionStorage.setItem('session', JSON.stringify(session));

// filepath for images
//let PATH = "https://cdn.glitch.com/f1c758c2-fd4c-4433-8a70-acaec9f76206%2F";
let PATH = "/Users/sanjanasharma/Desktop/collaborative-creation-app/level1/";
// hide results button
...
// images for level1: 1 is PHOTO, 0 is GAN
image_array = [
	{image: "lfw_000.jpg", value: 1},
	{image: "lfw_001.jpg", value: 1},
	{image: "e01_000.png", value: 0},
	{image: "e01_001.png", value: 0}
];
...
for (let j = 0; j < size; j++) {
  let img = shuffled_array[j].image;

  let startdiv = '<div class="card">';
  let enddiv = '</div>';
...
// Oh man this button does so much now... FIXME
// Original: https://codepen.io/maziarzamani/pen/oxBKxX

$('#result-btn').click(function(e) {

  // turn modal on (adding class 'activated' and prevent refresh of page	
  $('.modal').addClass('activated');
  e.preventDefault();

  // reverse array based on how card data is being served to viewers (ONLY ONCE!)
  shuffled_array = shuffled_array.reverse();
  for (let k = 0; k < size; k++) {
    // add comparison array to shuffled
    shuffled_array[k].userval = comparison_array[k];
    // add CORRECT/INCORRECT performance to shuffled
...
function chartData() {
  ...
  session1 = performance_data;
  // sanity check: print session data
  console.log("session1: " + session1);
  
  // unique name generator using round + round_number (stored in [0] of array)
  let id = "round" + JSON.stringify(performance_data[0]);
  console.log(id);

  // send session1 data to session storage with uniquename via id
  sessionStorage.setItem(id, JSON.stringify(session1));
}
...
async function chartIt() {
	...
	data: {
	    ...
	    datasets: [{
	        //label: ['Sensitivity', 'Specificity', 'Accuracy'],
